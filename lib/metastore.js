...
export default class MetaStore {
  ...
  async updateCatalog(path) {
    ...
    const sample_stream = await file.rows({ size: file.size }) 
    //TODO: Might be a bottleneck later
    const sample = await toArray(sample_stream)
   
    this._catalog[dataset._descriptor.title] = dataset._descriptor
    this._catalog[dataset._descriptor.title]['schema'] = schema
    this._catalog[dataset._descriptor.title]['sample'] = sample
  ...
  initMetaStoreFromGithub(catalog, descCatalog ) {
    this._catalog = catalog
    this.searchCatalog = descCatalog
  }
}